...
from code.train.train_models import extract_and_cluster, fitter, prepare_datasets, create_dataloaders, valid, save, \
    add_correlation_weight, add_cross_correlation_weight, add_fake_cross_correlation_weight
...
def do_train(
    ...
    for epoch in range(start_epoch, end_epoch+1):
        ...
        rgb_info, ir_info = add_correlation_weight(rgb_info, ir_info, args.neighbour, args.coe, args.r)
        ...
        if stage == 2:
            ...
            r2i_proxy, i2r_proxy = trainer.memory.get_cross_proxies()
            rgb_info, ir_info = add_cross_correlation_weight(rgb_info, ir_info, r2i_proxy, i2r_proxy, args.neighbour, args.coe, args.r)

        if stage == 1:
            logging.info(f'Epoch[{epoch}]==> Start cross modal matching')
            trainer.memory.creat_cross()
            r2i_proxy, i2r_proxy = trainer.memory.get_cross_proxies()
            rgb_info, ir_info = add_fake_cross_correlation_weight(rgb_info, ir_info, r2i_proxy, i2r_proxy, args.neighbour, args.coe)
