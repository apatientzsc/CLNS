...
class Trainer_stage1(Base_trainer):
    ...
    def _parser_data(item):
        ...
        return img, proxy, label, cam, correlations, weights
    ...
    def run(self, rgb_trainloader, ir_trainloader, magnification = 1):
        ...
        for iter_index in range(self.train_iters):
            ...
            img_rgb, proxy_rgb, label_rgb, cam_rgb, correlations_rgb, weights_rgb = self._parser_data(rgb_trainloader.next())
            img_ir, proxy_ir, label_ir, cam_ir, correlations_ir, weights_ir = self._parser_data(ir_trainloader.next())
            ...
            with self.amp_autocast():
                ...
                #CDL损失，代理损失，代替传统的类别损失
                loss_proxy_rgb, loss_proxy_ir, _, _ = self.memory(
                    feats_rgb, proxy_rgb, label_rgb, correlations_rgb, weights_rgb, None, None, None, None, None,
                    feats_ir, proxy_ir, label_ir, correlations_ir, weights_ir, None, None, None, None, None
                )
