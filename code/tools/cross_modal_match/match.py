...
def cross_modal_matching(k1, k2, proxies_features_rgb, proxies_features_ir, rgb_proxy_cam, ir_proxy_cam):
    ...
    if flag:
        similarity = torch.from_numpy(
            gnn_reranking(proxies_features_rgb, proxies_features_ir, rgb_proxy_cam, ir_proxy_cam, k1=k1, k2=k2)
        ).exp()
    else:
        similarity = torch.from_numpy(
            gnn_reranking(proxies_features_ir, proxies_features_rgb, ir_proxy_cam, rgb_proxy_cam, k1=k1, k2=k2)
        ).exp()
...
def otpm(k1, k2, proxies_features_rgb, proxies_features_ir, reg):
    ######################## OTPM #####################################

    num_cluster_rgb = proxies_features_rgb.shape[0]
    num_cluster_ir = proxies_features_ir.shape[0]
    ...
    if flag:
        similarity = ((torch.mm(proxies_features_rgb, proxies_features_ir.T)) / 1).exp().cpu()
        print(f"mm is used\n")
    else:
        similarity = ((torch.mm(proxies_features_ir, proxies_features_rgb.T)) / 1).exp().cpu()
        print(f"mm is used\n")
    ...
    result = ot.sinkhorn(a, b, M, reg, numItermax=5000, stopThr=1e-5)  #sysu:5.regdb:0.03.llcm:0.1
