...
def process_images(data_path, img_w, img_h, mode, dataset, data_type, trial=1):
    ...
    #img_list, id_list, cam_list, path_list = [], [], [], []
    ...
    return img_list, np.array(id_list), np.array(cam_list)#, path_list
...
def process_test_msvr310(data_path, mode, kind, trial=1, extend_dir='256x128'):
    mode_kind_map = {
        ('ir2rgb', 'gallery'): 'rgb',
        ('rgb2ir', 'gallery'): 'ir',
        ('ir2rgb', 'query'): 'ir',
...
def creat_test_data(args, trial=1, mode=None, kind=None):
    if args.dataset == 'SYSU' or args.dataset == 'LLCM':
        img, label, cam= process_images(data_path=args.data_dir, img_w=args.img_w, img_h=args.img_h, mode=mode,
                                                dataset=args.dataset, data_type=kind, trial=trial)
        #img, label, cam, paths = process_images(data_path=args.data_dir, img_w=args.img_w, img_h=args.img_h, mode=mode, dataset=args.dataset,data_type=kind, trial=trial)
    elif args.dataset == 'RegDB':
        img, label, cam = process_test_regdb(data_path=args.data_dir, trial=trial, mode=mode, kind=kind)
    elif args.dataset == 'msvr310':
        img, label, cam = process_test_msvr310(data_path=args.data_dir, trial=trial, mode=mode, kind=kind)
    else:
